import { avgPosition, positionImprovement, playerWinPctSparkline } from "./LeagueStatsUtils";
...
function computePlayerDerivedStats(playerStats, sortedWeeks, playerWinPctSparkline) {
...
function computeLeagueStats(sortedData, playerStats) {
  ...
  const { rollingAvg13, rollingAvg52, prevAvg13, prevAvg52 } = getRollingAverages(counts);
  const { playersAllTime, players52, players13 } = getUniquePlayerCounts(sortedData);
  ...
  const trophyShelfStats = getTrophyShelfStats(sortedData, qualified13, qualified52);
