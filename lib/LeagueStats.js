...
function getRollingAverages(counts) {
  ...
  const rollingAvg13 = avg(counts.slice(-14, -1));
  const rollingAvg52 = avg(counts.slice(-53, -1));
  const prevAvg13 = avg(counts.slice(-27, -14));
  const prevAvg52 = avg(counts.slice(-105, -53));
...
function getHonorsBoardStats(playerStats, qualified104) {
  ...
  const topBestWinPct = top10(
    [...qualified104].sort(
      (a, b) => Number(b.winPercentage104) - Number(a.winPercentage104),
    ),
  );
  const topBestAvgPosition = top10(
    [...qualified104].sort(
      (a, b) => Number(a.averagePosition104) - Number(b.averagePosition104),
    ),
  );
  const topBestAvgPoints = top10(
    [...qualified104].sort(
      (a, b) => Number(b.averagePoints104) - Number(a.averagePoints104),
    ),
  );
  return {
    topMostWins,
...
function getTableStats(sortedData, vpsLookup = {}) {
  const recentWeeks = sortedData.filter((w) => w.scores?.length >= 3);
  ...
  recentWeeks.forEach((w) => {
    const meta = vpsLookup[w.vpsId];
    if (!meta) return;

    const { maker, year, designer } = meta;
    ...
    if (!makerMap[maker])
      makerMap[maker] = { maker, totalPlayers: 0, weeks: 0 };
    makerMap[maker].totalPlayers += w.scores.length;
    makerMap[maker].weeks += 1;

    if (!eraMap[era]) eraMap[era] = { era, totalPlayers: 0, weeks: 0 };
    eraMap[era].totalPlayers += w.scores.length;
    eraMap[era].weeks += 1;

    if (designer) {
      if (!designerMap[designer])
        designerMap[designer] = { designer, totalPlayers: 0, weeks: 0 };
      designerMap[designer].totalPlayers += w.scores.length;
      designerMap[designer].weeks += 1;
    }
  });
  ...
  const topDesigners = Object.values(designerMap)
    .filter((d) => d.weeks >= 8)
    .map((d) => ({ ...d, avgPlayers: (d.totalPlayers / d.weeks).toFixed(1) }))
    .sort((a, b) => Number(b.avgPlayers) - Number(a.avgPlayers));

  return { topMakers, topEras, topDesigners };
}
...
function computePlayerDerivedStats(
  ...
  playerStats.forEach((userData) => {
    ...
    userData.totalPoints104 = sortedWeeks.slice(-104).reduce((sum, week) => {
      const score = week.scores.find((s) => s.username === userData.username);
      return sum + (score ? score.points : 0);
    }, 0);

    userData.weeksPlayed104 = sortedWeeks
      .slice(-104)
      .filter((week) =>
        week.scores.some((s) => s.username === userData.username),
      ).length;

    userData.averagePoints104 = (
      userData.weeksPlayed104 > 0
        ? userData.totalPoints104 / userData.weeksPlayed104
        : 0
    ).toFixed(2);

    const wins104 = sortedWeeks.slice(-104).reduce((sum, week) => {
      const score = week.scores.find((s) => s.username === userData.username);
      return sum + (score ? week.numberOfPlayers - score.position : 0);
    }, 0);
    const losses104 = sortedWeeks.slice(-104).reduce((sum, week) => {
      const score = week.scores.find((s) => s.username === userData.username);
      return sum + (score ? score.position - 1 : 0);
    }, 0);
    userData.winPercentage104 = (
      wins104 > 0 ? (wins104 / (wins104 + losses104)) * 100 : 0
    ).toFixed(2);

    const weeks104 = sortedWeeks.slice(-104);
    const played104 = weeks104.filter((w) =>
      w.scores.some((s) => s.username === userData.username),
    );
    userData.averagePosition104 = (
      played104.length > 0
        ? played104.reduce((sum, week) => {
            const score = week.scores.find(
              (s) => s.username === userData.username,
            );
            return sum + (score ? score.position : 0);
...
function computeLeagueStats(sortedData, playerStats, vpsLookup) {
  ...
  const qualified104 = playerStats.filter((p) => p.weeksPlayed104 >= 60);
  ...
  const honorsBoardStats = getHonorsBoardStats(playerStats, qualified104);
  ...
  const tableStats = getTableStats(sortedData, vpsLookup);
...
export default function LeagueStats(data, vpsLookup = {}) {
  ...
  const leagueStats = computeLeagueStats(sortedWeeks, playerStats, vpsLookup);
