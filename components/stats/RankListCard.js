            ...
            {sorted.map((item, i) => (
              ...
              >
                <td className="px-2 py-1.5 flex items-center gap-1.5 min-w-0">
                  <span className="text-orange-700 shrink-0 w-4">{i + 1}.</span>
                  <span className="text-stone-100 font-semibold truncate">
                    {item[nameKey]}
                  </span>
                </td>
                <td
                  className={`px-2 py-1.5 text-right tabular-nums ${sortCol === totalKey ? "text-orange-400 font-semibold" : "text-stone-400"}`}
                >
                  {Number(item[totalKey]).toLocaleString()}
                </td>
                <td
                  className={`px-2 py-1.5 text-right tabular-nums ${sortCol === weeksKey ? "text-orange-400 font-semibold" : "text-stone-400"}`}
                >
                  {item[weeksKey]}
                </td>
                <td
                  className={`px-2 py-1.5 text-right tabular-nums ${sortCol === avgKey ? "text-orange-400 font-semibold" : "text-stone-400"}`}
                >
                  {item[avgKey]}
                </td>
              </tr>
